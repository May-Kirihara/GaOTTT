...
Last updated: 2026-05-13 (Phase J Stage 2 完了 — recall に `persona_context` + `tag_filter` の explicit 引数を追加、seed step + final result の **両段階で force-inject** する。MCP/REST parity で両方公開。embedding 距離が遠い query でも tag 一致なら surface 保証。Phase K Stage 1 — Stellar Supernova Cohort。Phase J Stage 1 — Persona-Anchored Retrieval。Phase I Stage 3 — mass-gated query attraction。Phase G/H — FAISS write-behind, genesis kick + dream + Stage 0 priming, mass-aware / source-aware / dynamic / virtual FAISS の seed redesign)
