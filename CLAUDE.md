...
├── server/                     # app.py (REST, MCP parity) + mcp_server.py (MCP "gaottt", 27 ツール)
...
7. **★ 7 階層テストスイート (`tests/perf/`) で 仮説→実装→検証 ループの 検証 step を回す** — 新機能・hot path 変更・retrieval geometry 触ったときは **手動実行** (CI 自動化なし、deliberate な measurement):
   ...
   - Tier 1 (smoke): startup / 27 MCP tools / BM25 build
...
- 全 27 MCP ツール: [`docs/wiki/MCP-Reference-Index.md`](docs/wiki/MCP-Reference-Index.md)
