...
tests/perf/                     # ★ 7 階層テストスイート (Tier 1/3/4/5/6/7、38 ケース)。新機能・hot path 変更時は必ず実行 → docs/wiki/Operations-Performance-Testing.md
...
6. **Smoke（両トランスポートで動くかの end-to-end 確認）**:
   ...
   ```
7. **★ 7 階層テストスイート (`tests/perf/`) で性能・品質評価** — 新機能・hot path 変更・retrieval geometry 触ったときは **必ず** 実行:
   ```bash
   .venv/bin/python -m pytest tests/perf/ -q      # 38 tests / ~7.4s / ruff clean
   ```
   ...
   - Tier 3 (quality): engine.query top-K / source-mix dominance
   ...
   - Tier 6 (perf): p50/p95/p99 latency / ingest throughput / startup / compact
   - Tier 7 (regression): golden corpus (30 chunks、11 queries)

   破壊的変更や config 変更で perf 数値の baseline を取り直す場合:
   ```bash
   .venv/bin/python scripts/perf_baseline.py --label "<context>"  # → tests/perf/baselines/<UTC>_<sha>_<label>.json
   .venv/bin/python scripts/perf_diff.py                          # 直近 2 baseline の diff、>25% で exit 1
   ```
   retrieval 挙動の per-query 詳細を snapshot/diff したいときは:
   ```bash
   .venv/bin/python scripts/diag_recall.py snapshot --queries-file tests/perf/golden_corpus/queries.json --data-dir /tmp/xxx --out /tmp/before.json
   # ...変更後...
   .venv/bin/python scripts/diag_recall.py diff /tmp/before.json /tmp/after.json
   ```
8. **本番 isolated benchmark (実 RURI embedder で確認したいとき)**:
   ```bash
   rm -rf /tmp/gaottt-bench
   .venv/bin/bash scripts/run_benchmark_isolated.sh 200
   # 本番 DB は触らない。p50 < 50ms を必達 (StubEmbedder Tier 6 と別途、RURI 込みの実数値検証)
   ```
...
13. **`tests/perf/` で性能・品質確認** — retrieval geometry / config default / hot path に触れた変更なら、上記 7. の Tier 別ガイドに従って必ず実行。perf 数値が大きく動いたら `perf_baseline.py` で baseline を取り直し、判断根拠を [Operations — Performance Testing](docs/wiki/Operations-Performance-Testing.md) に追記
...
# ★ 7 階層 perf テストスイート (新機能/hot path 変更時に必須)
.venv/bin/python -m pytest tests/perf/ -q                 # 38 tests
.venv/bin/python -m pytest tests/perf/test_tier5_*.py -v  # 特定 Tier
...
- **テスト fixture でランダム埋め込みを使うと flaky** — `tests/integration/test_engine_archive_ttl.py:StubEmbedder` のようにトークンベースの決定論的埋め込みを使う。`tests/perf/_helpers.py:StubEmbedder` (md5-seeded 768-dim random、cosine ~noise) は **BM25 と FAISS を独立シグナルとして検証したい場面専用** — 同じトークンでも類似度が出ないので、トークン重なりで意味マッチを試したいなら archive_ttl 版を使う
- **Tier 3 quality test で engine.query top-K に surface match を期待するときは tag_filter 注入が必要** — random embedder cosine 雑音下で plain `engine.query` の final_score は mass + wave + cosine 支配で、BM25 寄与は seed pool 入場のみ。`recall(tag_filter=[...])` で Phase J Stage 2 injection + Phase L Stage 1 RRF re-rank 経路に乗せると BM25 surface match が top-K に来る。`tests/perf/test_tier3_retrieval_quality.py` の Path A (plain、widened top_k) vs Path B (tagged、tight top_k) 2 経路パターンを踏襲
