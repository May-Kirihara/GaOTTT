...
scripts/hooks/                  # ambient_recall.py (Claude Code UserPromptSubmit + opencode 共通 backend) / opencode-ambient-recall.ts (opencode plugin) / ★ save_candidates.py (Claude Code Stop hook、write-side 対称、auto_remember 経由、`GAOTTT_SAVE_CANDIDATES_EMIT=stdout` で opencode plugin から stdout-emit モードで叩ける) / ★ save_candidates_inject.py (UserPromptSubmit-inject、Stop hook が書いた state file を読んで次 prompt に注入) / ★ opencode-save-candidates.ts (opencode plugin、`chat.message` で前 turn 読み出し → save_candidates.py を stdout モードで spawn → block を当 turn message に append、Claude Code の 2-script bridge を 1 plugin に圧縮)
