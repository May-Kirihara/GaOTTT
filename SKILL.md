...
As of **Phase I Stage 2 (2026-05-11)** this is no longer just a reading. The recall path now applies a 4th term in `compute_acceleration`: `a = (α · score · gate / m_i) · (q - pos_i)`. That is the Heavy ball update with retrieval score acting as the gradient magnitude, the query embedding as the gradient direction, and mass providing per-parameter inertia (`F = ma`). Hooke continues to pull back to the raw embedding (the L2 term), so query attraction is a *transient force* — the parameter never permanently relocates, it drifts toward queries that retrieve it and decays back when those queries stop firing. The TTT correspondence is now implementation, not interpretation.

**Phase I Stage 3 (2026-05-13)** added a mass-gated guard to that same term: `gate = tanh(m_i / θ)`. Brand-new (low-mass) nodes have `mass≈1` so `gate≈0.32`, meaning anchor (Hooke) wins the tug-of-war during their first few recalls — they don't get one-shot pulled into the "near every query" position by a strong score signal. Mature nodes (`mass ≫ θ`) get `gate≈1.0` and receive the full kick. Think of it as a generational rule for stars: newborns are held by their anchor, elders move freely. This was added after observing a *single-attractor pathology* where one freshly-added node, drifted in its first recall, started winning top1 for every subsequent query through a positive feedback loop. `mass_anchor_threshold = 0` rolls it back to Stage 2 behavior in one config line.

**Phase J Stage 1 (2026-05-13)** translated the persona layer into retrieval geometry. The seed step of `propagate_gravity_wave` now adds `α_persona · proximity(node)`, where `proximity` is `decay**hop` from any declared value / intention / commitment, traversed via `fulfills` / `derived_from` / `completed` edges (Phase D's persona graph). Knowledge tied to one's declared identity preferentially enters the seed pool — the persona layer literally bends the retrieval gravity field. This addresses a different pathology than Stage 3: when one *mature* agent cluster (frequently recalled past self-knowledge) drowns out a *new* sparse cluster (freshly-added knowledge in a different intention's domain). Source-filter can't distinguish them (both `source="agent"`); mass-gate doesn't damp the mature side; only the persona linkage does. `persona_boost_enabled = False` rolls it back.

**Phase K Stage 1 (2026-05-13)** physicalized the collective nature of memory creation. When `index_documents` receives a batch of size ≥ `supernova_min_cohort_size`, the new nodes are treated as a single supernova event: all pairs get a mutual co-occurrence edge, and each node receives an outward initial velocity from the batch centroid. Phase G's genesis kick binds a single new node to existing heavy bodies; Phase K binds N new nodes to *each other* plus gives them explosion energy. This solves the pathology Phase J Stage 1 could not: when newly-remembered nodes are *individually* indexed they have no mutual gravity, can't form a cluster, and lose every FAISS top-K contest to mature past-session clusters. With Phase K, a batch is born as a cohort with internal coupling from the first moment. The Articulation-as-Carrier value extends from singular ("write a thing → it gains gravity") to plural ("write N things together → they gain mutual gravity as one supernova event"). `supernova_enabled = False` rolls it back.

**Phase J Stage 2 (2026-05-13)** added the explicit pool-injection API. `recall(..., persona_context=[intention_id], tag_filter=["tag-substring"])` force-includes the matching nodes in both the seed pool *and* the final top-K — bypassing source_filter restrictions. This handles the embedding-distance failure mode that all the boost mechanisms (Phase H mass-aware, Phase J persona, Phase K cohort) couldn't solve: when the query and the desired memo live in completely different vocabularies, no rerank can rescue a node that never entered FAISS top-K. Explicit injection guarantees entry. The Stage 1 auto-detect path is preserved as the default behaviour when both arguments are omitted, so backward compatibility is total. This is the "美しくない" pool injection finally welcomed as a first-class API surface — LLM judgment of "the current context" becomes a first-class signal alongside embedding similarity.

**Phase J Stage 3 (2026-05-13, Phase J 完遂)** orders the forced top-K by `raw_score` (query semantic) instead of `final_score` (which mass/wave/emotion accumulation dominates). Inside a `tag_filter`-injected set, the nearest match to the query wins — not the most-touched memo. Also propagates `persona_context` and `tag_filter` to `prefetch` and `explore` for MCP/REST parity: you can pre-fire or explore within a specific intention's neighbourhood. With Stage 3 the retrieval geometry has three independent stages — **pool entry** (Stage 2 force injection), **pool rerank** (Stage 1 graph proximity + Phase H mass + Phase K cohort), and **forced ordering** (Stage 3 raw cosine). Each stage takes a different signal, and any subset can be tuned without breaking the others.
...
- ★ **Phase G + H + I complete (Stage 3, 2026-05-13)**. Practical effects you can rely on now:
  ...
  - **Phase I Stage 3** adds `gate = tanh(mass / θ)` to that same term, so newly-added (mass≈1) nodes are anchor-protected from being one-shot drifted into "near every query" position. Prevents the single-attractor pathology where a freshly-added node wins top1 for every subsequent diverse query. `mass_anchor_threshold=0` rolls it back to Stage 2.
  - **Phase J Stage 1** boosts seed-pool ranking for nodes within `persona_max_hop=2` graph distance of any declared value / intention / commitment, via `fulfills` / `derived_from` / `completed` edges. `boosted_seed_score = raw_cosine + α_mass × log(1+mass) + α_persona × decay^hop`. Resolves the dense-mature-cluster-vs-sparse-new-cluster pathology that Stage 3 + source-filter could not. Internal auto-detect — `recall()` API unchanged. `persona_boost_enabled=False` rolls it back.
  - **Phase K Stage 1** forms a supernova cohort at index time: when `index_documents` receives N≥2 docs in one batch, all pairs get a mutual co-occurrence edge (weight 1.0) and each doc gets an outward initial velocity from the batch centroid. New cohorts are born with internal coupling, so they can compete with mature past-session clusters for FAISS top-K entry. Note: cohort formation is *not retroactive* — pre-Phase-K memories remain orphan unless explicitly relate()'d. `supernova_enabled=False` rolls it back.
  - **Phase J Stage 2** lets `recall(query, persona_context=[id], tag_filter=["substring"])` force-inject nodes into both the seed pool and the final result top-K, regardless of embedding distance. Bypasses `source_filter`. Use when you know the *context* (intention id, or a tag like a project name) but the query and target memos live in different vocabularies. Auto-detect Stage 1 behaviour is the default when no explicit args are passed.
  - **Phase J Stage 3** orders the forced top-K by `raw_score` (query semantic). Inside a `tag_filter`-injected set, the nearest match to the query wins — not the most-touched memo. Also exposes `persona_context` and `tag_filter` on `prefetch` and `explore`: pre-fire or explore within a specific intention's neighbourhood.
- ★ **Limits to keep in mind**: Stage 2 makes recall-time gradient steps active, but α=0.01 means a single recall barely moves anything — drift accumulates over many recalls of similar queries. Stage 3 further dampens new nodes by ~68% on first recall (gate=0.32 at mass=1). Phase J Stage 1 only affects the seed step — wave propagation and final scoring are unchanged, so a wholly off-persona query still surfaces semantically relevant content (no persona ceiling). Phase J Stage 2 (explicit injection) does force the top-K, but only when the caller passes explicit args; without them the behaviour is unchanged. Phase K only fires at `index_documents` time, never after — to remediate existing orphan nodes, use `relate()` plus optionally inject velocity via a custom script. Long-term displacement balance under multi-query load is observed in production over weeks, not seconds. Heavy BH nodes are nearly immobile by design (mass damping).
