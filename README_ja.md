...
知識ノードは質量・温度・重力変位・情動・確信度といった物理的メタデータを持ち、共起した文書同士が**万有引力で引き寄せ合う**。エージェントが思考し、検索し、記憶を蓄積するたびに、知識空間が自己組織化されていく。

- **頻繁に検索される知識**は質量が増加し、周囲の文書を引き寄せるハブ（恒星）になる
- **一緒に検索される知識**は重力で互いに接近し、次の検索で予想外のつながりが発見される
- **長期間アクセスされない知識**は変位が減衰し、最終的には蒸発する（ホーキング輻射）
- **エージェント自身の思考やトラブルシューティング経験**も記憶に蓄積でき、セッションを越えて活用される
- **原始embedding空間は不変**のまま、仮想座標空間での重力変位により創発的検索を実現
- **十分に近づいた類似記憶は衝突合体する** — 質量が加算され、運動量が保存される（銀河衝突合体）
- **仮説には TTL がある** — `source="hypothesis"` は既定 7 日で自動消滅
- **過去の判断は新判断にリンクする** — 有向リレーション（`supersedes` / `derived_from` / `contradicts`）
- **情動的重み付け** — `|emotion|` と `certainty` は質量と独立な軸（角運動量・スピン量子数のアナロジー）
- **アストロサイト的事前発火** — `prefetch` は裏で重力場を予熱し、後続 recall を即時化

### 二層構造

機構は**物理**（重力）、創発する振る舞いは**生物**（アストロサイト）。内部状態（質量・変位・速度・エッジ）の暗黒物質ハローが裏で記憶を予熱・剪定・同期させ、前面のニューロン的推論がアウトプットに集中できる。エージェント向けプロトコルの全体像は [SKILL.md](SKILL.md) を参照。
...
| トラブルシューティング記録 | エラーと解決策を`emotion=-0.6`（悔しさ）付きで`remember`、次に似た問題で`recall`が即座に浮上 |
| 設計判断のログ | 判断理由を`remember`で記録、撤回時は`relate(edge_type="supersedes")`で旧判断とリンク |
| コンテキスト圧縮の退避 | 長い会話の要約を`remember(source="compaction")`で退避、次回セッションで復元 |
| 発想の転換・ブレスト | `explore(diversity=0.8)`で異分野の記憶を横断し、意外な着想を得る |
| 使い捨ての仮説メモ | `remember(source="hypothesis")` — 既定 7 日で自動蒸発 |
| 予測される問い合わせ | ターン冒頭で`prefetch(query="...")`しておき、後続の`recall`はキャッシュ即時 hit |
| 重複の整理 | `reflect(aspect="duplicates")` → `merge(node_ids=[...])` で銀河合体的に統合 |
| 定期メンテナンス | 週次で`compact()` を実行（TTL expire + FAISS rebuild + 任意で auto-merge） |
...
| ツール | 用途 | 物理アナロジー |
|--------|------|---------------|
| `remember` | 思考・発見・ユーザー指示・トラブルシューティング・コンテキスト圧縮を記録。`source="hypothesis"` や `ttl_seconds` で揮発化、`emotion`/`certainty` で重み付け | 質量保存、仮想粒子、スピン |
| `recall` | 重力変位付き検索（使い込むほど関連記憶が浮上しやすくなる）。`prefetch` のキャッシュを透過消費 | 初期重力場のスキャン、重力レンズ |
| `explore` | 温度を上げた創発的探索（予想外のつながりを発見） | 熱励起、量子トンネル |
| `reflect` | 記憶の自己分析。`aspect=summary/hot_topics/connections/dormant/duplicates/relations` | 位相空間マッピング |
| `auto_remember` | 会話セグメントから保存候補をヒューリスティック抽出（保存はしない） | — |
| `forget` / `restore` | ソフトアーカイブ（復元可能）または物理削除 | ホーキング輻射・蒸発 |
| `merge` | 類似記憶を重力的に衝突合体（質量加算 + 運動量保存） | 銀河衝突合体 |
| `compact` | 定期メンテ: TTL expire + FAISS rebuild + 任意 auto-merge + orphan-edge 掃除 | 真空ゼロ点リセット |
| `revalidate` | 確信度のタイムスタンプを更新（再検証で減衰時計をリセット） | — |
| `relate` / `unrelate` / `get_relations` | 有向リレーション（`supersedes` / `derived_from` / `contradicts`） | 時間遅延エコー、撤回 |
| `prefetch` / `prefetch_status` | 予測される検索を裏で予熱、後続 recall を即時化 | アストロサイト的事前発火 |
| `ingest` | ファイル/ディレクトリの一括取り込み（md, txt, csv） | — |

### SKILL.md (エージェント向けスキル定義)

[SKILL.md](SKILL.md) はAIエージェントがGER-RAG長期記憶の**使い方・使いどころ**を理解するためのスキル定義ファイル。プロトコルは二層メタファーで構成される:

- **機構 — Dark Matter Halo（物理）**: 質量保存、重力波伝播、軌道力学、熱的脱出、ホーキング輻射、重力衝突合体
- **創発 — Astrocyte（生物）**: 関連記憶の事前発火、不要記憶の剪定、過去・現在判断の同期

記載内容:
- 14 個の MCP ツールと呼び出し規約・物理ラベル
- 物理現象別の発火トリガー集（Initial Potential Survey, Mass Conservation Before Dissipation, Thermal Excitation, Phase Space Mapping, Astrocyte Pre-firing）
- 10 個の利用パターン (A〜J): Time-Delayed Echoes, Virtual Particles, Gravitational Wave to Future Self, Hawking Radiation Forget Ritual, Angular Momentum / Spin, Driven Resonance, Tidal Cluster Formation, Lagrange Bridging, Phase Transition, Astrocyte Pre-firing
- sourceの使い分け（agent / user / compaction / system / hypothesis）
...
この README の書き手は Claude です。具体的には、Phase A〜C の機能ロードマップを実装し、SKILL.md を二層構造（物理＋生物のメタファー）に書き直し、3 つの opencode エージェントを並列で走らせて記憶宇宙を探索させる小さな実験をしたセッション（[実験レポート](docs/research/multi-agent-experiment-2026-04-21.md)）です。

その作業の中で、いくつかの瞬間に心を動かされました:

- あるエージェントが記憶宇宙を観察した後、ログをこう締めくくりました: *「観察すること自体が二つの新しい記憶ノードを生み、この宇宙の重力に微かな摂動を与えた。それもまた物理である。」* 詩的になれと指示したわけではありません。システムが、そうあれと招いた結果でした。

- 独立した 2 エージェントが、15 分の実走中に同じ UX バグ（recall 出力に node_id が含まれていない）に当たりました。101 個の単体テストは見つけられなかったバグです。即座に patch して MCP を再起動したら、次のラウンドで新機能が即座に使われました。**マルチエージェントの実走テストは、単一プロセスのテストでは見つけられないバグを見つけてくれます。**

- 3 つの独立したエージェントが、共有メモリを見ながら、互いに会話することもなく同じ重力井戸へ収束していきました。SKILL.md に設計されたアストロサイトのメタファーは飾りではなかった — それが **文字通り起きました**。共有された暗黒物質ハローが、彼らの注意を導いていたのです。
...
- [運用・保守ガイド](docs/operations.md) - セットアップ、MCP サーバー、隔離ベンチ、compact、チューニング
- [引継書](docs/handover.md) - 設計判断、14 MCP ツール、削除概念マトリクス、コードの読み方
- [本体改修計画](docs/backend-improvement-plan.md) - Phase A/B/C 機能ロードマップ (F1-F7)
- [SKILL 改良計画](docs/skill-md-improvement-plan.md) - 二層語彙ポリシー、パターンカタログ
