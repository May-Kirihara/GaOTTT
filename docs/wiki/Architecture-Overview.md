...
| **7 階層テストスイート (2026-05-14)** | `tests/perf/` に Tier 1/3/4/5/6/7 を 38 tests 整備、各 Tier は対応する事件群 (FAISS 空、WAL 暴走、RRF scale 不整合、hub chunk 独占) が起こったときに鳴る。Stage 1 (Tier 1+5+7 雛形) → Stage 2 (Tier 3+4+`scripts/diag_recall.py`) → Stage 3 (Tier 6+`scripts/perf_baseline.py`+`perf_diff.py`+`.github/workflows/perf-tests.yml`) を同日完遂。全テストは `tmp_path` + StubEmbedder で本番 DB 不可触、~7.4 秒で 38 件 ruff clean | 新機能・hot path 変更時に **必ず実行**。設計案 memory id=55579286、Stage 1 完了 id=c356abe5、Stage 2/3 完了 id=3eb313b9、commitment id=faf61f5f。**実 RURI で latency 絶対値を測りたいときは `scripts/run_benchmark_isolated.sh` を併用** (両者は補完関係、後者は p50 < 50ms 目標を実 embedder で検証する用)。詳細: [Operations — Performance Testing](Operations-Performance-Testing.md) |
