...
- **P2 — グラフ衛生**: 🟢 **機構実装済 (2026-06-02、[Lateral Association Stage 8](Plans-Ambient-Recall-Lateral-Association.md))、default OFF**。`cache.get_association_strength(node, mode, hub_degree_cut)` が degree-normalized 連想強度 (cosine `w/√(deg·deg)` / PPMI) + explicit degree percentile cut を提供、Stage 5 resonance が consumer。**本番 re-check の学び (2026-06-02)**: soft cosine だけだと「フロイト hub」は消えるが survivors が依然 moderate-high degree の bulk chunk → **`hub_degree_cut` の percentile が決定的レバー** (p70 で deg=6 の「future-self への手紙」が *Articulation as Carrier* に novel-far surface したが 11→1 と aggressive)。残: **percentile tuning (p80-90)** を measurement で詰める。
- **P3 — データ / 時間**: agent×agent エッジを **organic な多様 recall** から積む必要がある。一括 recording セッションは dense で低品質な clique を作る。時間をかけて自然な co-recall が貯まるか、recording 由来の session-clique を別途減衰させる。P2 の soft 正規化の効きも、bulk session artifact が薄まるほど上がる。

→ 最短ルートは **P2 の percentile tuning を Lateral Association Stage 8 で本番計測 (measurement-first)**、衛生済みの共起強度で「specific な連想が top に来る」を確認してから Accretion mode 本体に着手する。Accretion gather は `get_association_strength` をそのまま流用する (P2 の上に乗る)。
