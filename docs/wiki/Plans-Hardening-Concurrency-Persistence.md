...
| ID | 箇所 | 内容 |
|---|---|---|
| M1 | `sqlite_store.py:136/320/510 等` | `IN (?,?,...)` が SQLite 999 変数上限を無視 → corpus-scale ingest/forget で `OperationalError`、dedup 失敗時は重複書込。`_in_chunks(ids, fn, 900)` で全 call site。 |
| M2 | `memory.py:264-277`, `reflection.py:55-94` | reflect/dormant/summary が 24k ノードで `await store.get_document` を逐次 N 回、event loop ブロック。バッチ `get_documents`/`count_by_source` + `asyncio.sleep(0)` interleave。 |
| M3 | `sqlite_store.py:523-546` 等 | 多文 destructive op が明示 transaction 外 → 途中例外で部分適用 + 未 rollback。`BEGIN`/`COMMIT`+`rollback`。 |
| M4 | `sqlite_store.py:395` | `save_displacements` dtype 無検査 → float64 で無言ゴミ化。`np.ascontiguousarray(disp, dtype=np.float32)`。 |
| M5 | `bm25_index.py:89-209` | tombstone 無限増加、`search` が毎クエリ active postings 再構築 → uptime で p50/p95 劣化。removed 比率 20% で自動 rebuild。 |
| M6 | `gravity.py:278/282` | friction 係数が負を取りうる (`orbital_friction>1` で velocity 毎ステップ反転 runaway)。`max(0.0, 1-friction)` + config range 検証。 |
