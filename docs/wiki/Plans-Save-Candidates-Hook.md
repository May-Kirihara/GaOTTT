...
- **install path**: グローバルなら `~/.config/opencode/plugin/`、プロジェクトなら `<project>/.opencode/plugin/` に `.ts` を symlink (`ln -s /mnt/holyland/Project/GaOTTT/scripts/hooks/opencode-save-candidates.ts ~/.config/opencode/plugin/`)。opencode 起動時に自動ロード
