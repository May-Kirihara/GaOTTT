...
> 状態: **全 Stage 0-4 実装済 (2026-05-31, branch `feat/instruction-surface-hygiene`)** — GLM-5.1 委託 + Opus 検証で完遂、未 push。commit: `f594352` (S0 token baseline + S1 版ラベル除去) / `a8526a4` (S3 decision tree + cross-lingual dedup) / `bf188e2` (S2 ambient content budget + trailer gating) / `90c1709` (S4 hook harness + parity)。検証: 全 suite 803 passed / 1 skipped (FAISS write-behind 2本が full-suite で稀に flaky、単独再実行で pass)、mcp_smoke + rest_smoke green、marker ガード round-trip 決定的。**発見**: (1) caller 向けテキスト (S0/1/3) は token 総量不変 = signal-to-noise 改善であって削減ではない。(2) C2「ambient は青天井」前提は不正確 — service が既に `ambient_excerpt_chars`=240 で cap 済み、S2-a の default は後方互換 no-op で knob は 240 より下に絞る運用者向け。残り = レビュー → push/PR。
