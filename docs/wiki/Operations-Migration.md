...
| **M004 !** | phase-m-cosmic-bang | 全 active node に Phase K supernova の outward velocity (`α × (emb - centroid)`、`orbital_max_velocity` で clamp) を付与 — 既存ノードに初期運動量を一回限り注入して emergent dynamics の点火を行う | 全 active node の 50% 以上が velocity ≈ 0 の時に PENDING 判定。M002 で velocity が wipe された直後の `100%` がデフォルトの想定。既存 velocity に **加算** して clamp、自然累積を保存。新規 batch には Phase K が常時効くので index 時点では発火しない |
