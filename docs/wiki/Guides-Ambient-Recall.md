...
- **▼ ささやき** — Observation Apparatus Refinement Stage 2 (2026-05-26 追加)。`mode="dormant"` で取り出される「使われていない・古い memo」(`mass ≤ dormant_mass_threshold`、`age ≥ dormant_age_threshold_seconds`、`source ∈ dormant_source_classes`) のうち、query に対して **BM25 で強一致** したもの 1 件を whisper として混ぜる。これは「〇〇といえば〜だったよな」感覚を機構化する slot — 場が手放しかけていたが、語が触れて 1 度だけ surface する。一致しなければ slot は空 (`silence beats off-topic noise`)。force computation / mass update は一切触らない (counter-importance sampling)。`ambient_dormant_relevance_floor` (既定 `0.5`) 以上の BM25 score が必要、`ambient_dormant_slot_count` (既定 `1`) で件数上限、`ambient_dormant_slot_enabled=False` で完全 off。manifest にも `dormant=` キーで載るので hook の rotation list が次 turn の重複を防ぐ。
- **▼ ⚠ 矛盾** — surface 記憶の `contradicts` エッジのペア。
- **▼ いま誰として** — active な declared value/intention（grounding）。スロットを埋めるのは「mass 上位を query 関連度で再ランクした top1」: 候補プール (mass 上位 N、既定 N=10) を `score = (mass ** w) × cos(query, persona_vec)` で再ランクし、`cos < ambient_persona_min_relevance` なら slot を空にする ([Plans — Ambient Recall Refinement](Plans-Ambient-Recall-Refinement.md) Stage 1)。`w = ambient_persona_mass_weight` (既定 `0.3`、2026-07-02 昇格) は heavy persona dominance の対処 knob — production で 1 つの persona の mass が突出している場合、`0.5` (緩やか) / `0.0` (cos のみ) とさらに下げて cos 軸の影響を強める、逆に `1.0` で Stage 1 (mass 完全寄与) に戻す ([Operations — Troubleshooting](Operations-Troubleshooting.md) 「query 横断で同じ persona に固定される」節)。
...
Observation Apparatus Round 2 ([Plans](Plans-Observation-Apparatus-Round-2.md) Stage E) で導入された 2 つの slot 選別 lens。2026-06-12 dogfooding で direct hits 6 件中 5 件が chat-history raw chunk (state-dict ダンプ・無関係コード断片・依頼定型句一致) だった観察への対処。**2026-06-12 に両方とも default ON へ昇格** (本番実機評価 id=fa1d03a8 で意図どおりの分離を確認、旧 ambient が会話ログ raw chunk に支配されて実用性が低かった経緯)。

| knob | default | 効果 |
|---|---|---|
| `ambient_conversational_source_factor` (E1) | `0.5` = ON (`1.0`=OFF) | `ambient_conversational_sources` (既定 openai / claude-web / claude-code) の item の slot ranking score を factor 倍。novelty decay と同じ combined multiplier に合成、direct + lensing のみ (persona slot 不触) |
| `ambient_dump_symbol_ratio` (E2) | `0.45` = ON (`>=1.0`=OFF) | content 先頭 400 字の dump score (`max(記号比率, 長 ASCII 識別子トークン比率)`) が閾値超の候補を slot から除外、次点繰り上げ (自然文 <0.05 / readable code ≤0.40 / state-dict 羅列 >0.97) |

どちらも **観測・注入層のみ** — recall 本体 / force / mass には不適用 (Phase M 単一規則不変。precedent: `exclude_tags`、novelty decay)。rollout は Stage 7 anti-hub と同じ 2 段階を踏んだ: code merge (OFF) → env/config opt-in で dogfooding → 実機評価で効果確認 → code default 昇格 (本ターン)。legacy 挙動に戻すには両 knob を `1.0` に。
...
`.claude/` は gitignore 対象なので、フック*スクリプト*（`scripts/hooks/ambient_recall.py`）はリポジトリで共有されるが、フック*登録*は各自の settings.json に書く。

- **global**: `~/.claude/settings.json` — 全レポで Claude Code を起動したときに hook が effective (GaOTTT 以外のレポでも自動で記憶が引かれる)
- **per-project**: `<project>/.claude/settings.json` — そのプロジェクトでだけ on

どちらにせよ **path は絶対パスで書く** こと。`$CLAUDE_PROJECT_DIR` を使うと Claude Code が「いま開いている project の dir」に展開するので、GaOTTT 以外のレポで起動した瞬間に hook script が見つからず `operation blocked by hook` で agent 動作が止まる (2026-05-27 production 発覚)。

`/Path/to/GaOTTT` を実際の GaOTTT clone の絶対パスに置き換え:
...
> GaOTTT working tree が `main` (or `scripts/hooks/ambient_recall.py` を含む branch) に check out されていることも確認 — 古い branch のままだと disk から script が消えていて、上記と同じく hook block が起きる。
...
| `GAOTTT_AMBIENT_NOVELTY_TURNS` | `5` | 両方 | 直近 N 個の ambient block の `<!-- ambient-ids ... -->` manifest を parse し、`{node_id: count}` を `recently_surfaced` として server に forward する（[Lateral Association Stage 1](Plans-Ambient-Recall-Lateral-Association.md)）。Claude Code は transcript の `hook_success` attachment から、opencode は過去 user message text に append された ambient block から抽出。server 側で各 slot の ranking が `ambient_novelty_decay ** count` で減衰し、直近で出た memo が rotation する。`0` で disable (= 旧挙動・無 decay) |
