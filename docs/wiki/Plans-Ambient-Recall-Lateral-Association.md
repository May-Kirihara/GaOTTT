...
## Stage 8 — Degree-Normalized Association Strength (共起グラフ層の anti-hub) — 🟢 実装完了 (default OFF, 2026-06-02)

> [Accretion Recall](Plans-Accretion-Recall.md) の measurement-first gate (2026-06-02、`scripts/diag_assoc_halo.py` 本番計測) が炙り出した **P2 前提条件**。Accretion 専用ではなく Lateral Association 自身の改善でもあるのでここに置く — 「〇〇といえば〜」の核は *promiscuous でない specific な連想* を surface することで、それを壊しているのが共起 weight の **生 count** だから。Stage 7.1 が **ranking 層** の cluster-monoculture を叩くのに対し、Stage 8 は **共起グラフ層** の hub promiscuity を叩く。失敗モードが別 (下表)。

### 観察された問題 — 生 co-occurrence count は hub を区別しない

`cache.get_neighbors(node)[other]` が返す weight は **co-recall の生カウント**。一括 recording セッション (自己知識 139 件を同時 recall 等) や長セッションで何度も surface した promiscuous hub は、**意味的関連と無関係にあらゆる anchor に対し高 weight** を持つ。weight でランク or pull する consumer は hub を増幅する。

**実測 (2026-06-02、`diag_assoc_halo.py --source-filter agent ...`)**: self-authored anchor の連想ハロー上位に、同じ高 weight ノード (「フロイト…」w=16) が **観測者効果クエリにも超新星コホートクエリにも**出現。novel-far の reachability は ✅ だが中身が hub noise で、「〇〇といえば〜」の **specificity 軸**を破っていた。共起グラフ全体は cross-doc edge 16% (agent×agent 15k) だが、その多くが session co-occurrence artifact。

| 失敗モード | 層 | 対策 stage |
|---|---|---|
| 同一 cluster (cohort/original) が複数席を占める | ranking (slot 構成) | Stage 7.1 (cluster_key MMR) |
| 高次数 hub が「何にでも関連」して見える | 共起グラフ (weight 自体) | **Stage 8 (degree 正規化)** |

### 設計 — co-occurrence weight を association strength に変換

生カウント `w(a,b)` を、両端の次数で割った **連想強度**に変換 (共起グラフ上の cosine / TF-IDF 類比):

```
assoc(a, b) = w(a, b) / sqrt( deg(a) · deg(b) )        # cosine 正規化 (5a 候補)
```
または PMI 類比:
```
assoc(a, b) = log( w(a, b) · W / ( deg(a) · deg(b) ) ) # pointwise mutual information (5b 候補)
```
`deg(x) = Σ_n w(x, n)` (x の総 co-recall mass)、`W = Σ all weights`。次数の大きい hub は全相手への assoc が割り引かれ、**a としか共起しない rare で specific なノードは高 assoc を保つ** = 「〇〇といえば〜」が欲しい信号そのもの。`deg` は cache load 時に precompute (次数は緩変、recall ごとの再計算不要)。

**任意の追加 anti-hub**: `deg(b)` が active 分布の P パーセンタイル超のノードを gather から落とす explicit cut (Stage 7.2 の percentile 思想を degree 軸に流用)。

### 適用先

- **Consumer 1 (既存・今 in-scope)**: Stage 5 `_cooccurrence_resonance` (`services/memory.py:536`) は今 **生 `get_neighbors` カウント**を resonance に使っており、同じ hub bias を被っている。これを assoc strength に切替えると lensing の trust signal が hub を信頼しなくなる → Stage 8 は単体で Stage 5 を改善する。
- **Consumer 2 (将来・Accretion の P2 前提)**: `explore(mode="associative")` の gather 層が halo を assoc strength で rank / pull し、high-degree hub を percentile cut する。Accretion はこの上に乗る。

### 不変条件 / 単一規則

- **physics (force / mass update) に一切触らない。** 共起グラフ由来の観測 / ranking signal の変換のみ。Stage 7.1 と同じ posture (ranking layer)。
- **source 分岐ゼロ。** degree は構造量 (source-blind)。`is_self_force_by_id` gate (同一 original/cohort 除外) の **後段**に合成する — gate が internal-trade を落とし、Stage 8 が残りの hub promiscuity を割り引く、という二段。
- **knob**: `cooccurrence_assoc_normalization: str = "none"` (既定 = 生カウント、bit-exact legacy) / `"cosine"` / `"pmi"`、+ optional `cooccurrence_hub_degree_percentile_cut: float | None = None`。両 default で挙動完全不変。

### Acceptance / 計測

- `scripts/diag_assoc_halo.py` を拡張: halo 各メンバーの `deg` を出力 + assoc strength で再ランクし、**hub が落ちて query-specific 連想が上がるか**を目視 + 数値化。Accretion go/no-go はこの再ランク後に再走 (NOT YET → 再判定)。
- unit: cosine / pmi 正規化の形・hub down-weight・`"none"` で生カウント一致。integration: Stage 5 resonance が hub ノードを以前より低く評価することを StubEmbedder で確認。
- 本番 acceptance: 上記「フロイト…」hub が self-authored anchor の上位 novel-far から消え、cos 0.80 の「自発的な鏡」intention のような meaningful な連想が相対的に浮上するか。

### 実装 (2026-06-02, default OFF)

| 項目 | 実装 |
|---|---|
| Config | `cooccurrence_assoc_normalization: str = "none"` ("none"/"cosine"/"pmi"、`GAOTTT_*` env 可) + `cooccurrence_hub_degree_percentile_cut: float \| None = None` (JSON-only) |
| `store/cache.py` | lazy degree map (`deg(x)=Σw`) + total weight、`get_degree()` / `get_association_strength(node, *, mode, hub_degree_cut)`。set_edge/remove_edge/load_from_store/evict_node/reset で invalidate。`"none"` は `get_neighbors` のコピー (bit-exact) |
| Consumer | `services/memory.py::_lensing_resonance` (Stage 5) を生カウント → assoc strength に切替。`"none"` で legacy 完全互換。**正規化 ON 時は `ambient_lensing_resonance_scale` 要再調整** (正規化値は raw count より遥かに小さいレンジ) |
| テスト | `tests/unit/test_cooccurrence_assoc.py` 8 件 — none bit-exact / degree invalidation / cosine が hub を specific 以下に降格 / PPMI clamp / hub-cut / resonance が legacy では hub と specific を区別不能・cosine で hub 降格。全 755 passed |
| 診断 | `scripts/diag_assoc_halo.py` に `--assoc-mode` / `--hub-degree-cut` + degree カラム |

### 本番 re-check (2026-06-02、`diag_assoc_halo.py` self-authored anchor)

| mode | top novel-far 例の中身 | 判定 |
|---|---|---|
| `none` | 「フロイト…」(raw w=16, deg≈105) が無関係クエリ横断で top | hub 支配 (gate 前) |
...
- [Accretion Recall](Plans-Accretion-Recall.md) — Stage 8 を P2 前提として要求する retrieval-geometry 層の想起機構
- Stage 5 (lensing resonance) / Stage 7.1 (cluster anti-hub) — Stage 8 の consumer / 姉妹 anti-hub
- [`scripts/diag_assoc_halo.py`](../../scripts/diag_assoc_halo.py) — degree / assoc 再ランクを measurement する診断
...
- [Accretion Recall (降着想起)](Plans-Accretion-Recall.md) — Stage 8 を P2 前提とする retrieval-geometry 層版の「〇〇ってなんだっけ」機構
