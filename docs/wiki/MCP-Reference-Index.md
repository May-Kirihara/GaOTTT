...
GaOTTT（旧 GER-RAG）が公開する **28 個の MCP ツール** の早見表。MCP サーバー名は `gaottt`（旧 `ger-rag-memory`）。詳細は各カテゴリページに。
...
| `save_candidates` | Stop hook 用 — auto_remember を `<gaottt-save-candidates>` block で返す | [Memory](MCP-Reference-Memory.md) |
...
└── Stop hook 候補 surface → save_candidates (block を次 prompt 注入)

何かを思い出したい
├── 関連記憶              → recall
├── id が分かっている     → get_node (read-only、重力場不変)
