...
- [Phase O — TTT Observability](Plans-Phase-O-TTT-Observability.md) — LLM caller を TTT loop の participant に昇格させる observability layer。Phase I Stage 2 で literal 成立した「retrieval = gradient step」を caller-side に閉じる。5 stage: score breakdown / training delta trailer / query routing / list mode / dormant surface。**Stage 1 + 2 完了 (2026-05-14)**: Stage 1 で forward pass (gradient) を、Stage 2 で backward pass (parameter update) を caller に露出。`ScoreBreakdown` + `TrainingDelta` Pydantic model、MCP/REST 両側 parity、`expose_score_breakdown=False` / `training_delta_enabled=False` で legacy fallback、`cache_hit` で「update を suppress した」を「触れる node 無し」と区別可能。Stage 3-5 未着手
