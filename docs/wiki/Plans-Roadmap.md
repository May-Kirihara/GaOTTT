...
- **Phase N (未起草、候補が複数競合中)** — Phase N は予約地で、以下 3 つが「Phase N 候補」として記録されている。どれが最初に Plans 化されたら Phase N 確定、残りは Phase P/Q/R に繰り下げる:
  - **(N-α) RRF-scale aware mass boost** — Phase L Stage 1 (RRF) と Phase H Stage 1 (`α × log(1+mass)`) の score scale 不整合 (2026-05-14 発見: cosine スケール想定の α が RRF スケール ~0.03 に対し過剰、mass の重い無関係 chunk が semantic を上書き) を構造的に解消。暫定対処は `wave_seed_mass_alpha = 0.0` で seed boost 完全 disable ([Operations — Troubleshooting](Operations-Troubleshooting.md) §「ファイルで登録した文書が recall に出てこない」)。設計案: (a) RRF score の正規化 / (b) rank-based boost / (c) Phase H の意図 (heavy node lift) を別レイヤーに移す。`wave_initial_k=3` の見直し (大規模 corpus に対し小さすぎる) も同時検討。
  - **(N-β) Mass Evaporation (Hawking radiation 類比)** — mass の出力側を作る別軸機構。Phase M で「自己関与は mass を生まない」の入力側制約を literal 化したので、対称形として「使われない mass は自然減衰する」の出力側を Phase 化する候補。Phase M Stage 1 適用後、1-2 週の自然蓄積を観測したのち、**過剰に accumulate する mass があれば** 着手の signal となる ([Plans — Phase M §13](Plans-Phase-M-Mass-Conservation.md#13-future-work--phase-n-mass-evaporation-hawking-radiation))
...
- [Phase O — TTT Observability](Plans-Phase-O-TTT-Observability.md) — LLM caller を TTT loop の participant に昇格させる observability layer。Phase I Stage 2 で literal 成立した「retrieval = gradient step」を caller-side に閉じる。5 stage: score breakdown / training delta trailer / query routing / list mode / dormant surface。**Stage 1 + 2 + 3 完了 (2026-05-14)**: Stage 1 で forward pass (gradient) を、Stage 2 で backward pass (parameter update) を、Stage 3 で query routing (caller の surface form classifier + reflect 並走) を caller に露出。`ScoreBreakdown` / `TrainingDelta` / `RoutingHint` Pydantic model、MCP/REST 両側 parity、`expose_score_breakdown=False` / `training_delta_enabled=False` / `auto_route_enabled=False` で legacy fallback、`cache_hit` で「update を suppress した」を「触れる node 無し」と区別可能、`routing_hint.pattern_matched` と `.auto_routed` を分離 — 「router が off」と「pattern に非ヒット」を区別可能。Stage 3 の classifier は **query 形式 (surface form) ベース、source 分岐ゼロ** で Phase M の単一規則を侵さない。Stage 4-5 未着手
