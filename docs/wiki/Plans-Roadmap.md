...
| **Phase J** | Persona-Anchored Retrieval — declared identity が retrieval geometry を曲げる | ✅ Stage 1 完了（2026-05-13）— Phase I Stage 3 acceptance での「自己言及的攻撃」観察から設計、実装は graph traversal + seed step boost。本番 acceptance で「seed pool 入場権」問題が露呈 (boost は pool 内 rerank、pool injection しない)、Phase K へ |
