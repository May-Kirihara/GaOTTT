...
| **Phase L** | Embedder ensemble / Query expansion / Hybrid retrieval — embedder の semantic ranking と LLM 意味解釈のズレを埋める | 🔮 検討中（2026-05-13 設計動機の発見）— Phase J 完遂後の本番 acceptance で「Surface 7/7 ✅ / Top1 semantic 整合 0-1/7 ⚠️」と分離。Phase J 三段構造の射程外、embedder (RURI v3 310m) の hidden ranking が dominant signal という構造的境界が見えた。候補軸: BM25+RURI+e5 の union seed、LLM query rephrase、forced 内 LLM rerank |
