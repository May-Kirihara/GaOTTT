...
- [Phase O — TTT Observability](Plans-Phase-O-TTT-Observability.md) — LLM caller を TTT loop の participant に昇格させる observability layer。Phase I Stage 2 で literal 成立した「retrieval = gradient step」を caller-side に閉じる。5 stage: score breakdown / training delta trailer / query routing / list mode / dormant surface。**Stage 1 完了 (2026-05-14)**: `ScoreBreakdown` Pydantic model + MCP/REST 両側で `score_breakdown` 露出、`expected_sum` プロパティで `final_score` を additive に再現、`expose_score_breakdown=False` で legacy fallback。Stage 2-5 未着手
