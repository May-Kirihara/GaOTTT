...
| **Phase J** | Persona-Anchored Retrieval — declared identity が retrieval geometry を曲げる | ✅ Stage 1 + Stage 2 完了（2026-05-13）— Stage 1: graph traversal + seed step boost。Stage 2: explicit `persona_context` + `tag_filter` の API、seed step + final result の両段階で force-inject、MCP/REST parity で公開。embedding 距離が遠い query でも tag 一致なら surface 保証 (pool injection の literal な実装) |
