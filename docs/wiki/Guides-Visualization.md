...
# Phase Q — 各 lively ノードが自 anchor を中心に描く閉軌道 (公転) を重ねる
.venv/bin/python scripts/visualize_3d.py --sample 3000 --orbital-trails --open
```
...
| 軌道トレイル (Phase Q) | 自 anchor 中心の閉楕円を sphere に貼った琥珀ループ (`--orbital-trails`、PCA 専用) | 同左 (sphere に貼らず素の楕円) |
...
| 琥珀色の閉ループ | 軌道トレイル (Phase Q) | 各 lively ノード (`\|v\|>0.001`) が **自分の anchor を中心に描く閉軌道 (公転)** — default では **非描画**、`--orbital-trails` で opt-in |

## 軌道トレイル (Phase Q) — `--orbital-trails`

各 lively ノードが **自分の原始 embedding (anchor) を中心に描く閉じた楕円** を琥珀色の faint なループで重ねる。Hooke アンカー `F = -k·d` は **等方調和振動子** (Bertrand の定理で閉軌道を生む 2 つの中心力の 1 つ) なので、その解析解

```
d(θ) = cos(θ)·d₀ + sin(θ)·(v₀/ω),   ω = √(orbital_anchor_strength)
```

を 1 周分サンプルして `anchor + d(θ)` を描く。接線速度 (`orbital_tangential_alpha > 0`) があれば `d₀` と `v₀` が非共線になり本物の楕円、角運動量ゼロなら anchor を貫く直線に縮退する (Bertrand の line-through-origin)。近傍重力がこの楕円を歳差させてロゼットになるので、描いているのは **osculating (接触) 軌道** = その瞬間のスナップショット。

- **公転中心は常に自分の anchor** — 衛星化 (他者を中心に回る = anchor migration) は描かない。Phase M 単一規則の「線の内側」を視覚的にも literal に守る。
- **PCA 専用** — 線形射影が必要 (anchor/disp/vel を同一 transform に通す)。`--method umap` 指定時や `orbital_anchor_strength ≤ 0` のときは空 (描画なし)。
- anchor は絶対位置なので PCA の `mean_` を引く、disp/vel は差分ベクトルなので mean が相殺される。
- 詳細思想: [Plans — Phase Q](Plans-Phase-Q-Orbital-Mechanics.md) / [Operations — Tuning §公転・閉軌道](Operations-Tuning.md)。
