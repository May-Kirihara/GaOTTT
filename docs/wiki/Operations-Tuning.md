...
| `cooccurrence_hub_degree_percentile_cut` | `None` | **Lateral Association Stage 8**: explicit な anti-hub floor。degree が active 分布の P パーセンタイル超の neighbor を association strength 返却前に drop (Stage 7.2 mass percentile cut の degree 軸版)。`None`=cut なし。本番 re-check (2026-06-02) で **soft 正規化 (cosine/pmi) 単独では bulk-content hub を崩しきれず、この cut が決定的レバー**と判明 — `p70` で deg=6 の specific 連想 (future-self への手紙) が surface したが kept halo 11→1 と aggressive、`p80-90` を `scripts/diag_assoc_halo.py --assoc-mode cosine --hub-degree-cut <P>` で measurement-first に詰める。`\| None` 既定ゆえ JSON-only (env 非対応) |
