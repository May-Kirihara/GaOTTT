...
| dormant_mass_percentile | `None` | **Lateral Association Stage 6.2 (2026-05-26)**: 分布相対の mass cut。設定すると `dormant_mass_threshold` の絶対値 (`2.0`) を捨て、active corpus mass の P パーセンタイル値を cut として使う。`None` (既定) で legacy 絶対値挙動。production で「26k memo の mass 分布が上振れし `≤ 2.0` が常に 0 件」となる失敗 (`project_phase_o_stage_5_production_observation`) への対処。本番値は `scripts/diag_dormant.py --data-dir <prod>` で「~5-15 dormant 候補が surface する最小パーセンタイル」を採用 (典型は 10-30 の範囲) |
...
| `direct_hit_anti_hub_lambda` | `0.0` | **Lateral Association Stage 7.1 (2026-05-26)**: direct-hit anti-hub。`ambient_recall.direct` と `recall` の top-K 構成で同一 cluster key (`cohort_id` OR `original_id`、後者は file ingest の chunk グループ化に使われる) の連続採用にペナルティ `λ × count_of_shared_cluster` をかけ、greedy MMR で並び替え。両キーとも None (pre-Phase-M 旧 memo のみ) は penalty 対象外。`0.0`=既定 (挙動不変)、`0.4`=本番候補 (`tests/perf/test_tier3_cluster_monoculture.py` で `avg_unique 2.67→4.00 / avg_max_dom 2.33→2.00` を確認、本番でも 638-chunk 本クラスター `米国会社四季報` が top-5 で 1/5 に capped を確認 2026-05-26)、`1.0` 近辺=強い分散。Phase M 単一規則整合 (cluster identifier は両方とも構造的 id、source/tag 分岐なし)。**limitation**: individual-node high-mass dominance (一握りの singleton agent/intention/commitment memo が cluster 関係なく top-K を占有する production 観察) には効かない、それは Phase N Mass Evaporation の領域。詳細: [Plans — Lateral Association Stage 7](Plans-Ambient-Recall-Lateral-Association.md) |
