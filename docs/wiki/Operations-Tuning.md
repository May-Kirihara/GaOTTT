...
| `ambient_conversational_source_factor` | `1.0` (OFF) | **Observation Apparatus Round 2 E1 (2026-06-12)**: ambient slot (direct / lensing) の ranking score を、`ambient_conversational_sources` に含まれる source の item に対し factor 倍する乗算 lens。novelty decay と同じ combined multiplier (`final_score × novelty × src_factor`) に合成。`1.0`=OFF (bit-exact legacy)、推奨 opt-in `0.5`。背景: 2026-06-12 dogfooding で direct hits 6 件中 5 件が chat-history raw chunk (state-dict ダンプ・依頼定型句一致)。観測・注入層のみ — recall 本体 / force / mass には不適用 (Phase M 単一規則不変)。env `GAOTTT_AMBIENT_CONVERSATIONAL_SOURCE_FACTOR` |
...
| `ambient_dump_symbol_ratio` | `1.0` (OFF) | **Observation Apparatus Round 2 E2 (2026-06-12)**: dump-shape gate。content 先頭 400 字の dump score = `max(記号比率, 長 ASCII 識別子トークン比率)` がこれを超える候補を ambient slot (direct / lensing) から除外、次点繰り上げ。state-dict キー羅列・生コード断片の注入防止。`>= 1.0`=OFF (E1 と同じ規約 — None default だと GAOTTT_* env 自動マップが効かない)、推奨 opt-in `0.45` (日本語/英語の自然文は <0.2、state-dict 羅列は >0.7)。env `GAOTTT_AMBIENT_DUMP_SYMBOL_RATIO` |
