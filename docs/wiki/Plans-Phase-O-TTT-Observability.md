...
> 状態: **Stage 1 + Stage 2 実装完了 (2026-05-14)、Stage 3-5 未着手**。Stage 1 (Score breakdown) + Stage 2 (Training delta trailer) で TTT loop の forward (gradient) + backward (parameter update) の両側が caller に literal に見えるようになった — 設計同型 (`retrieval = gradient step`) が caller を含めて閉じた最初の状態。次は Stage 3 (Query routing)。
