...
> 状態: **Stage 1 + Stage 2 + Stage 3 実装完了 (2026-05-14)、Stage 4-5 未着手**。Stage 1 (Score breakdown) + Stage 2 (Training delta trailer) + Stage 3 (Query routing / recall+reflect auto-merge) で TTT loop の forward / backward / 認知負荷削減の 3 面が caller に literal に渡るようになった。設計同型 (`retrieval = gradient step`) は caller を含めて閉じ、surface-form mismatch の罠 (Phase I Stage 4 本番 acceptance で観察された「現在 active な commitment は?」→ recall で startup-diagnostic 提案) は classifier で吸収される。次は Stage 4 (List mode) / Stage 5 (Dormant surface)。
