...
| 4. Dynamics | 時間で壊れない | anti-hub / displacement runaway / 世代安定性 / **driven resonance / diversity 配線** / **Phase Q 公転（clamp 有界・閉楕円・lively 自己制限）** | hub chunk 独占、displacement 暴走、diversity no-op、Phase I Stage 2 query-kick 退行、orbit tick の発散 / 楕円潰れ / lively set 非有界（連続 tick コスト暴走） | `test_tier4_dynamics.py`, `test_tier4_phase_o_resonance.py`, `test_tier4_phase_o_diversity.py`, `test_tier4_phase_q_orbital.py` |
