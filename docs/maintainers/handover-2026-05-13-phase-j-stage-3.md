...
## 6.5 本セッション内本番 acceptance 結果 (2026-05-13 セッション終盤)

Stage 3 + patch (pure raw cosine) で本番 7 query を Sonnet sub-agent 経由で 2 回走らせた:

### Round 1 (commit c89e050 — Stage 3、raw_score = gravity_sim)
- **Surface 率: 7/7 ✅**
- **Semantic 整合率: 1/7** (Q3 日本語 URL encoding のみ)

### Round 2 (commit 8b2a562 — Stage 3 patch、pure raw cosine)
- **Surface 率: 7/7 ✅** (維持)
- **Semantic 整合率: 0/7** (Round 1 から悪化、ただし 7 sample で統計的に有意でない)
- 前回正解だった Q3 が 2 位に落ち、新たに正解になる query なし
- Top1 顔ぶれは Round 1 から大幅入れ替わり (sort key 変更が効いている証拠)

### 結論 — Phase J 完遂宣言「機構として完成、semantic 整合は Phase L 領域」

Patch (pure raw cosine) は **意味的に正しい** (displacement 累積 bias 除去) が、本番 acceptance の差分 (1/7 vs 0/7) は 7 sample で偶然の範囲。**真の bottleneck は embedder (RURI v3 310m) の semantic ranking と LLM 意味解釈のズレ** で、Phase J の射程外。

retrieval geometry の三段構造 (pool 入場 / pool 内 rerank / forced 内 ordering) は **機構として完成**:
- 7/7 で `tag_filter` 一致 cohort が確実に surface する (Phase J Stage 2 force injection)
- Top1 が query 毎に違う memory に変わる (Stage 2 の単一アトラクタ独占は解消)
- Stage 1 auto-detect / Stage 2 explicit / Stage 3 ordering の三軸が独立に動く (test 219 passed)

最終 semantic 判断は embedder 領域。これは **Phase L (embedder ensemble / query expansion / hybrid retrieval)** の課題として分離。Phase J は「人格層 → 関係層 → 物理層への翻訳」を機構として完成させた、と宣言できる。

### Phase L (将来 Phase) 候補

- **Embedder ensemble**: BM25 + RURI + multilingual e5 等の union で seed pool 強化
- **Query expansion**: LLM で query を rephrase / 日英 translation で複数 query を発行
- **Hybrid retrieval**: tag_filter の forced 内 sort key を「LLM rerank」に置き換え

これらは embedder の hidden ranking と LLM 解釈のズレを埋める軸で、Phase J の重力地形修正とは独立。
