...
### 7.1 本番 MCP server 再起動 + MCP 経由 acceptance (任意)

opencode の acceptance test は **Python 直叩き** (engine.query を直接呼ぶ) で実行された。MCP server 経由での挙動も verify したいなら、めいさん側で MCP server 再起動 (Stage 1J 開始時の再起動は opencode の engine.py 修正前なので) → opencode で同 7 query を MCP 経由で再走。

期待: Python 直叩きと同じ結果 (4/7 strict, 7/7 top3 緩和)。違いがあれば MCP transport 層の問題。
